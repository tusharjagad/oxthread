...
- **AI Monitoring System** (`src/lib/ai-monitor.ts`, `src/lib/resource-analysis.ts`, `GET /api/azure/alerts`, `POST /api/azure/alerts/[id]`) — automated cost analysis agent using per-resource month-over-month comparison. Detects: **cost hikes** (resources with >50% cost increase month-over-month with severity critical/high/medium), **idle-but-billed resources** (resources with <15% cost variance across months and >$5/mo — identified as always-on but underutilized), **daily cost spikes** (>2σ above 14-day rolling average), and **new resources** (appearing this month costing >$10). Alerts are deduplicated by type + resourceName and surfaced on the cost dashboard with severity badges, detailed trace information (previous/current cost, change %, variance), and acknowledge/dismiss controls.
