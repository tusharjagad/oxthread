...
- **AI Monitoring System** (`src/lib/ai-monitor.ts`, `src/lib/resource-analysis.ts`, `GET /api/azure/alerts`, `POST /api/azure/alerts/[id]`) — automated cost analysis agent using per-resource month-over-month comparison. Detects: **cost hikes (monthly)** (resources with >50% cost increase month-over-month with severity critical/high/medium), **idle-but-billed resources** (resources with <15% cost variance across months and >$5/mo — identified as always-on but underutilized), **daily cost spikes** (today's cost >2σ above 14-day rolling average), and **new resources** (appearing this month costing >$10). Alert descriptions include time window labels (month-over-month, daily-vs-14day-avg) and traceable before/after amounts. Alerts are deduplicated by type + resourceName and surfaced on the cost dashboard with collapsible display (top 5 shown, "Show all N" toggle).
- **Cost API resilience** (`GET /api/azure/costs`) — each Azure API call (current month, last month, forecast) wrapped with `.catch()` returning null so a single failing query doesn't 500 the whole endpoint.
- **Advisor recommendations fix** (`src/lib/azure-cost.ts`) — resource name now extracted from `resourceMetadata.resourceId` path instead of `rec.id` GUID, fixing hash-ID display names and $0 impact values.
- **Login page cleanup** (`/app/login/page.tsx`) — removed "All Systems Operational" status indicator and "Sign in to your account to continue" subtitle; reduced heading size; removed button gap animation causing layout shift/chop; removed chevron bounce and lock rotation.
- **Azure API resilience** (`src/lib/azure-cost.ts`) — added `fetchWithRetry()` wrapper (2 retries with backoff) to all Azure Management API calls to handle transient 429 rate-limits. Recommendation resource names now extract from `resourceMetadata.resourceId` or fall back to `extendedProperties.sku`/`savingsTarget` instead of showing hash GUIDs.
- **Premium cost UI** (`/dashboard/azure/costs`) — redesigned stats cards with accent-color support matching main dashboard; AI Monitoring Alerts now click-to-expand showing detection timestamp, resource name, metric values, and threshold; recommendations filter out $0-impact subscription-level items; chart empty states show helpful messages instead of accusatory error text.
- **Resource Analysis API** (`GET /api/azure/costs/analysis`) — returns per-resource cost increases (>20%), decreases (<-20%), and idle resource candidates with full cost history for tracing on the dashboard.
- **Dashboard sections** (`/dashboard/azure/costs`) — new "Resource Cost Hikes" card (red border, per-resource with % change and $ amounts) and "Idle Resources — Billed but Underutilized" card (amber border, with cost/variance breakdown and reasoning).
